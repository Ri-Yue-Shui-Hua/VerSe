# -*- coding : UTF-8 -*-
# @file   : SPPELAN_UNet3D.py
# @Time   : 2024-05-21 17:33
# @Author : wmz
import torch
import torch.nn as nn


class SPPELAN_UNET3D(nn.Module):
    def __init__(self,
                 in_channels,
                 num_classes,
                 attention=False,
        ...
        self.sppelan = SPPELAN(c1=16, c2=16, c3=32)
        # Downsamplers
...
class SPPELAN(nn.Module):
    ...
    def __init__(self, c1, c2, c3, k=5):
        ...
        self.cv2 = nn.MaxPool3d(kernel_size=k, stride=1, padding=k // 2)
        self.cv3 = nn.MaxPool3d(kernel_size=k, stride=1, padding=k // 2)
        self.cv4 = nn.MaxPool3d(kernel_size=k, stride=1, padding=k // 2)
...
if __name__ == "__main__":
    ...
    model = SPPELAN_UNET3D(in_channels=1, num_classes=1, attention=True).to(device)
    ...
    #     input_names = ['input']
    #     output_names = ['output']
    #     export_onnx(model, input, input_names, output_names, "spp_elan_unet3d.onnx")
